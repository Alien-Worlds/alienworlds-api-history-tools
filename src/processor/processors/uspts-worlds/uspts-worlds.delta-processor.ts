...
export default class UsptsWorldsDeltaProcessor extends DeltaProcessor<ContractData> {
  public async run(
    ...
    try {
      ...
      const repository = await Ioc.setupUsptsWorldsDeltaRepository(mongoSource);

      if (table === UsptsWorldsTableName.UserPoints) {
        deltaModel.data = Entities.UserPoints.fromStruct(data);
      } else {
        /*
